    ...
    Window_OmoriFileInformation.prototype.refresh = function(valid, info, id) {
      ...
      if (valid) {
        if(this._select){
          ...
          // Draw Actor Face
          let faceSpriteData = actor.dummyrevFaceName ? actor.dummyrevFaceName : actor.faceName
          //console.log(actor);
...
info.actorData = {name: "REVERIE SAVE FILE", dummyrevName: actor.name(), level: actor.level, faceName: actor.faceSaveLoad(), faceIndex: actor.faceSaveLoadIndex(), dummyrevFaceName: actor.faceSaveLoad2()};
...
if (actor) {
  let faceName = actor.dummyrevFaceName ? actor.dummyrevFaceName : actor.faceName
  // Set Bitmap
...
Window_OmoriFileStats.prototype.updateStats = function (valid, info, id) {
  ...
  this.contents.drawText('FILE ' + id + ':', 1, 1, 130, this.contents.fontSize, 'center');
  if (valid) {
    this.contents.drawText(info.dummyChapter, 1, 31, 130, this.contents.fontSize, 'center');
    var actor = info.actorData;
    // Draw Actor Name
    this.contents.fontSize = 24;
    // Draw Level
